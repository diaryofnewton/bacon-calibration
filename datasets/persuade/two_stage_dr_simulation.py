"""
Two-stage adaptive sampling simulation for mean score estimation on PERSUADE.

Design:
1) Stage-1 label sampling: S1_i ~ Bernoulli(pi)
2) Fit outcome model f using labeled stage-1 essays
...
5) Refit outcome model on all sampled essays and estimate mean via DR:

    mu_hat = mean_i [ f_i + S_i * (Y_i - f_i) / q_i ]
    with q_i = pi + (1-pi)*sigma_i

Baselines per trial:
    (B1) Uniformly sample exactly round(eta * N) essays, report sample mean.
...
def build_feature_table(
    ...
    include_llm_features: bool,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, list[str]]:
    judge_path = results_dir / "mrr_persuade_judge_oof_scores_uncertainty_emb_intercept.csv"
    llm_path = results_dir / "llm_scores.jsonl"
    emb_path = results_dir / "openai_text_embedding_3_large_essay_embeddings.npz"

    # Compatibility fallback for older layout (files under data/).
    if not emb_path.exists():
        alt_emb = data_dir / "openai_text_embedding_3_large_essay_embeddings.npz"
        if alt_emb.exists():
            emb_path = alt_emb
    if not llm_path.exists():
        alt_llm = data_dir / "llm_scores.jsonl"
        if alt_llm.exists():
            llm_path = alt_llm

    judge_df = pd.read_csv(judge_path)
    y_df = judge_df[["id", "y_true"]].copy()
    y_df["id"] = y_df["id"].astype(str)
    y_df.drop_duplicates(subset=["id"], inplace=True)

    # Embeddings + PCA for shared essay features (available without labels).
    emb = np.load(emb_path, allow_pickle=True)
    emb_ids = [str(x) for x in emb["essay_ids"]]
    Xemb = emb["embeddings"].astype(np.float32)
    emb_cols = [f"emb_{j}" for j in range(Xemb.shape[1])]
    emb_df = pd.DataFrame(Xemb, columns=emb_cols)
    emb_df.insert(0, "id", emb_ids)

    merged = y_df.merge(emb_df, on="id", how="inner")
    if merged.empty:
        raise RuntimeError("No overlapping ids between judge labels and embeddings.")

    X = merged[emb_cols].values.astype(np.float32)
    scaler = StandardScaler()
    Xz = scaler.fit_transform(X)

    judge_pca_n = min(judge_pca_components, Xz.shape[1], Xz.shape[0] - 1)
    judge_pca = PCA(n_components=judge_pca_n, random_state=42)
    Xp_judge = judge_pca.fit_transform(Xz).astype(np.float32)
    judge_cols = [f"pcj_{j}" for j in range(Xp_judge.shape[1])]
    judge_df = pd.DataFrame(Xp_judge, columns=judge_cols)
    judge_df.insert(0, "id", merged["id"].tolist())

    sigma_pca_n = min(sigma_pca_components, Xz.shape[1], Xz.shape[0] - 1)
    sigma_pca = PCA(n_components=sigma_pca_n, random_state=42)
    Xp_sigma = sigma_pca.fit_transform(Xz).astype(np.float32)
    sigma_cols = [f"pcs_{j}" for j in range(Xp_sigma.shape[1])]
    sigma_df = pd.DataFrame(Xp_sigma, columns=sigma_cols)
    sigma_df.insert(0, "id", merged["id"].tolist())

    features = judge_df.copy()
    ...
    if include_llm_features:
        ...
        with llm_path.open("r", encoding="utf-8") as f:
            for line in f:
                ...
                rows.append(
                    {
                        "id": str(r["essay_id_comp"]),
                        "model": str(r["model"]),
                        "score": float(pred),
                        "entropy": float(r.get("mean_entropy", np.nan)),
                        "ppl": float(r.get("perplexity", np.nan)),
                    }
                )
        llm_df = pd.DataFrame(rows)
        llm_df = llm_df[llm_df["id"].isin(features["id"])].copy()

        score_w = llm_df.pivot_table(index="id", columns="model", values="score", aggfunc="mean")
        entropy_w = llm_df.pivot_table(index="id", columns="model", values="entropy", aggfunc="mean")
        ppl_w = llm_df.pivot_table(index="id", columns="model", values="ppl", aggfunc="mean")
        model_names = sorted(score_w.columns.tolist())

        wide = pd.DataFrame(index=score_w.index)
        for m in model_names:
            ...
            wide[f"s__{m}"] = s
            wide[f"sxe__{m}"] = s * e
            wide[f"sxlp__{m}"] = s * lp
        wide = wide.reset_index()
        features = features.merge(wide, on="id", how="left")

    full = merged[["id", "y_true"]].merge(features, on="id", how="left").merge(
        sigma_df, on="id", how="left"
    )
    full.replace([np.inf, -np.inf], np.nan, inplace=True)

    ids = full["id"].astype(str).to_numpy()
    y = full["y_true"].to_numpy(dtype=np.float32)
    feat_cols = [c for c in full.columns if c not in {"id", "y_true"}]
    X_full = full[feat_cols].to_numpy(dtype=np.float32)
    X_sigma = full[sigma_cols].to_numpy(dtype=np.float32)  # sigma uses essay features only
    ...
    *,
    ...
    for r in range(restarts):
        ...
        with torch.no_grad():
            ...
            cand = {
                "sigma": sig_all.detach().cpu().numpy(),
                "g": g_val,
                "obj": obj_val,
            }
            if best is None:
...
def run_trial(
    ...
    # Baseline (B1): direct uniform sample of size eta*N and take mean.
    ...
    # Baseline (B2): same direct sample, fit judge model, then DR with constant q=m/N.
    ...
    baseline2_dr = float(
        np.mean(
            f_base_all
            + s_base.astype(np.float32) * (y - f_base_all) / np.clip(q_base, 1e-6, None)
        )
    )

    s1 = rng.binomial(1, pi, size=n).astype(bool)
    if s1.sum() < 30:
        # Ensure enough points to fit the stage-1 outcome model.
...
def _init_worker(
    ...
    global _W_Y, _W_X_FULL, _W_X_SIGMA, _W_PI, _W_ETA, _W_SIGMA_L2_BETA, _W_SIGMA_DEVICE
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Two-stage DR simulation on PERSUADE.")
    ...
    parser.add_argument("--pca-components", type=int, default=64, help="Legacy shared PCA size.")
    ...
    parser.add_argument("--include-llm-features", action="store_true")
    ...
    parser.add_argument("--n-jobs", type=int, default=1, help="Parallel workers for Monte Carlo trials.")
    parser.add_argument(
        "--threads-per-worker",
        type=int,
        default=1,
    ...
    parser.add_argument("--out-prefix", type=str, default="persuade_two_stage_dr_sim")
    ...
    judge_pca_components = (
        args.judge_pca_components if args.judge_pca_components is not None else args.pca_components
    )
    sigma_pca_components = (
        args.sigma_pca_components if args.sigma_pca_components is not None else args.pca_components
    )

    ids, y, X_full, X_sigma = build_feature_table(
        data_dir=data_dir,
        results_dir=results_dir,
        judge_pca_components=judge_pca_components,
        sigma_pca_components=sigma_pca_components,
        include_llm_features=args.include_llm_features,
    )
    _ = ids  # kept for potential future row-level output expansion.
    ...
    if args.n_jobs <= 1:
        ...
        if (not args.no_progress) and (tqdm is not None):
            iterator = tqdm(iterator, total=args.trials, desc="Simulation trials", unit="trial")
        for s in iterator:
        ...
        progress = None
        if (not args.no_progress) and (tqdm is not None):
            progress = tqdm(total=args.trials, desc="Simulation trials", unit="trial")
    ...
    tag = f"{args.out_prefix}_pi{args.pi}_eta{args.eta}_T{args.trials}"
